...
void	child_builtin(t_prompt *prompt, t_mini *n, int l, t_list *cmd)
{
	...
	if (!is_builtin(n) && n->full_cmd)
		...
		g_status = mini_pwd();
	else if (is_builtin(n) && n->full_cmd && !ft_strncmp(*n->full_cmd, "echo", l) && l == 4)
		g_status = mini_echo(cmd);
	else if (is_builtin(n) && n->full_cmd && !ft_strncmp(*n->full_cmd, "env", l) && l == 3)
	{
...
void	*check_to_fork(t_prompt *prompt, t_list *cmd, int fd[2])
{
	...
	else if (!is_builtin(n) && ((n->full_path && !access(n->full_path, F_OK)) || dir))
		g_status = 126;
